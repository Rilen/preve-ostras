...
import { getWeatherData, type WeatherData } from '../services/dataService';
...
const TourismScore: React.FC = () => {
    ...
    useEffect(() => {
        const fetchWeather = async () => {
            ...
            if (data.length > 0) {
                // Pega o registro mais recente (assumindo que estão ordenados por data)
                setLatestWeather(data[data.length - 1]);
            }
        };
        fetchWeather();
    }, []);

    // Cálculo simples de score para demonstração
    const calculateScore = (w: WeatherData) => {
        let score = 100;
    ...
    const scoreValue = latestWeather ? calculateScore(latestWeather) : 75;

    return (
        <div className="glass-card p-8 rounded-3xl flex flex-col justify-between h-full min-h-[450px] relative overflow-hidden group">
            <div className="absolute -bottom-10 -right-10 w-40 h-40 bg-yellow-500/5 blur-[50px] rounded-full group-hover:bg-yellow-500/10 transition-colors"></div>
            ...
            <div>
                <div className="flex justify-between items-start mb-8">
                    <div>
                        ...
                        <p className="text-slate-400 text-sm mt-2 ml-13 font-medium">
            ...
            <div className="flex flex-col gap-6">
                {/* Score Circular Display / Main Metric */}
                <div className="flex items-center justify-between p-6 bg-slate-950/40 rounded-3xl border border-white/5 relative overflow-hidden">
                    <div className="flex items-center gap-5">
                        <div className="text-5xl drop-shadow-lg">
                            {scoreValue > 80 ? '🏖️' : scoreValue > 50 ? '⛅' : '🌦️'}
                        </div>
                        <div>
                    ...
                    <div className="text-right">
                        <div className={`font-black text-4xl tracking-tighter ${scoreValue > 80 ? 'text-emerald-400' : scoreValue > 50 ? 'text-yellow-400' : 'text-rose-400'
                            }`}>
                            {scoreValue}
                            <span className="text-lg font-bold text-slate-600 ml-1">/100</span>
                        </div>
                        <div className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-1">Preve-Ostras Index</div>
                    </div>
                </div>

                {/* Second Scenario Mock */}
                <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 bg-slate-950/20 rounded-2xl border border-white/5 hover:bg-slate-900/40 transition-colors">
                        ...
                        <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
                            <div className="h-full bg-gradient-to-r from-emerald-500 to-teal-400 animate-pulse" style={{ width: '65%' }}></div>
                        </div>
                    </div>
                    <div className="p-4 bg-slate-950/20 rounded-2xl border border-white/5 hover:bg-slate-900/40 transition-colors">
                        <div className="flex items-center justify-between mb-2">
                        ...
                        <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
                            <div className="h-full bg-gradient-to-r from-blue-500 to-indigo-400" style={{ width: '45%' }}></div>
                        </div>
                    </div>
                </div>
            </div>
